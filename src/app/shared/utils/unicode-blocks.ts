/**
 * Curated Unicode block ranges (start/end code point pairs), covering every
 * Basic Multilingual Plane block plus the supplementary-plane blocks most
 * commonly encountered in real text (emoji, math, CJK/Egyptian extensions,
 * musical/game symbols). The full Unicode registry defines ~327 blocks;
 * this list is intentionally a practical subset, not exhaustive — a code
 * point outside every listed range falls back to "Unassigned / Other".
 * Shared by Unicode Character Inspector and Unicode Table.
 */
...
export const UNICODE_BLOCKS: readonly UnicodeBlock[] = [
  ...
  // Selected supplementary-plane blocks (not exhaustive — see file doc comment).
  ...
  { name: 'Supplementary Private Use Area-A', start: 0xf0000, end: 0xffffd },
  { name: 'Supplementary Private Use Area-B', start: 0x100000, end: 0x10fffd },
];

const UNKNOWN_BLOCK = 'Unassigned / Other';

/** Binary search over the sorted, non-overlapping `UNICODE_BLOCKS` ranges. */
export function blockNameOf(codePoint: number): string {
  let low = 0;
