...
import { pipelineStep } from './image-format-converter.pipeline-step';
...
// that the step never throws — not the actual conversion, which needs a real browser.
const ONE_PIXEL_PNG_BASE64 =
  'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=';

describe('image-format-converter pipeline step', () => {
  it('rejects non-file input', async () => {
    const result = await pipelineStep.run({ type: 'text', value: 'nope' });
    expect(result).toEqual({ ok: false, error: { message: 'Image Format Converter expects file input.', kind: 'invalid-input' } });
  });
